...
public class LEDStrip {
    ...
    public static final int shooterMotorDiag = 0X01; //1
    /** Left segment 2 */
    public static final int shooterHoodDiag = 0X02; //2
    /** Left segment 3*/
    public static final int unused = 0X04; //4
    /** Left segment 4*/
    public static final int climberDiag = 0X08; //8
    /** Left segment 5*/
    public static final int gyroDiag = 0X10; //16
    /** Left segment 6 */
    public static final int apriltagDiag = 0X20; //32
    /** Left segment 7 (top) */
    public static final int spareDiag = 0X40; //64

    /** Right segment 1 (starts at top) */
    public static final int swerve1Diag = 0X80; // 128
    /** Right segment 2 */
    public static final int swerve2Diag = 0X100; // 256
    /** Right segment 3*/
    public static final int swerve3Diag = 0X200; // 512
    /** Right segment 4 */
    public static final int swerve4Diag = 0X400; // 1024
    /** Right segment 5 */
    public static final int hopperFloorDiag = 0X800; // 2048
    /** Right segment 6 */
    public static final int intakeDeployDiag = 0X1000; // 4096
    /** Right segment 7 */
    public static final int intakeWheelsDiag = 0X2000; // 8192

    /** All segments blue */
    public static final int allClear = 0X3FFF; //16383
    ...
    public void diagnosticLEDmode(){
        if (diagnosticPattern == allClear) { // if all is good, go entirely blue
            ...
            if((diagnosticPattern & shooterMotorDiag) != 0) {
                for(int i = 0;i<=3;i++)
                    ledBuffer.setRGB(i, 0, 255, 0); 
            }
            if((diagnosticPattern & shooterHoodDiag) != 0) {
                for(int i = 4;i<=7;i++) 
                    ledBuffer.setRGB(i, 0, 255, 0); 
            }
            if (true) { //unsued segment
            //if((diagnosticPattern & unused) != 0) {
                for(int i = 8;i<=11;i++) 
                    ledBuffer.setRGB(i, 0, 255, 0); 
            }
            ...
            if((diagnosticPattern & hopperFloorDiag) != 0){
                for(int i = 46; i<=49;i++)
                    ledBuffer.setRGB(i, 0, 255, 0);
            }
            if((diagnosticPattern & intakeDeployDiag) != 0){
                for(int i = 50; i<=54;i++)
                    ledBuffer.setRGB(i, 0, 255, 0);
            }
            if((diagnosticPattern & intakeWheelsDiag) != 0){
                for(int i = 55; i<=59;i++)
                    ledBuffer.setRGB(i, 0, 255, 0);
            }                        
        }
